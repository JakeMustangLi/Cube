...
def getResults(state):
    ...
    # filename = "states.pkl"
    # outFileLoc_pre = "../testData/%s/%s_nnetPar%s_depthP%s_bfs%s_%s_solved" % (
    # args.env.lower(), filename.split("/")[-1].split(".")[0], args.nnet_parallel, args.depth_penalty, args.bfs,
    # "_".join(methods))
    #
    # if args.endIdx != -1:
    ...
    #
    # state = states[args.startIdx:args.endIdx]
    ...
    sys.stderr.write("State: %i, %s" % (idx, solveStr))
